...
class NetUtils
{
    ...
    public static function curlData($RequestType = 'GET', $url, $parameter/* ,$post_data =array(),$is_proxy,$header="0",$cookie="",$isHttps=false */)
    {
        ...
        if ($isHttps || 
            (isset($proxy_ip)) && is_null($proxy_ip) && !empty($proxy_ip)
            && isset($proxy_ip[TableUtils::getTableDetails('proxy_ip','http_type')])
            && $proxy_ip[TableUtils::getTableDetails('proxy_ip','http_type')]==1
            ) {
            // https开启下面这个
    ...
    public static function parseTaobaokeKeyWords($parameter)
    {
        ...
        if (isset($bodyStr) && ! empty($bodyStr) && $parameter['ref']['status']==self::$SUCCESS) {
            ...
            if (isset($bodyObj) && is_array($bodyObj) && count($bodyObj) > 0 && isset($bodyObj['ret']) && is_array($bodyObj['ret']) && count($bodyObj['ret']) > 0 && ! empty($bodyObj['ret'][0]) && substr($bodyObj['ret'][0], 0, 7) != 'SUCCESS') {
                ...
                if (isset($matches) && is_array($matches) && count($matches) > 1) {
                    ...
                    if(isset($parameter['tokenObj']) && !is_null($parameter['tokenObj'])  && !empty($parameter['tokenObj'])){
                        $dataUpdate=[TableUtils::getTableDetails('tbk_token','status')=>1,
                            TableUtils::getTableDetails('tbk_token','update_time')=>time()
                        ];
                        //var_dump($parameter['tokenObj']) ;
                        Db::table(TableUtils::getTableDetails('tbk_token'))
                        ->where(TableUtils::getTableDetails('tbk_token','id'), $parameter['tokenObj'][TableUtils::getTableDetails('tbk_token','id')])
                        ->update($dataUpdate);//更新失效的tk
                    }
                    $data=[
                    ...
                    if(isset($bodyObj['proxy_ip']) && is_null($bodyObj['proxy_ip']) 
                        && is_array($bodyObj['proxy_ip']) && count($bodyObj['proxy_ip'])>0 ) {
            ...
            isset($parameter['ref']['status']) && $parameter['ref']['status']==self::$FAIL){
                
                if(isset($parameter['ref']['msg']) && strpos(trim($parameter['ref']['msg']) , 'Could not resolve proxy:')!=-1
                    && isset($parameter['proxy_ip']['id']) && !empty($parameter['proxy_ip']['id'])
                    ){
                    $dataUpdate=[TableUtils::getTableDetails('proxy_ip','status')=>2,
                        TableUtils::getTableDetails('proxy_ip','update_time')=>time(),
                        TableUtils::getTableDetails('proxy_ip','log')=>trim($parameter['ref']['msg']),
                    ];
                    
                    Db::table(TableUtils::getTableDetails('proxy_ip'))
                    ->where(TableUtils::getTableDetails('proxy_ip','id'), $parameter['proxy_ip'][TableUtils::getTableDetails('proxy_ip','id')])
                    ->update($dataUpdate);//更新失效的tk
            }   
        }
        return $parameter;
